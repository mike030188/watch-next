...
class LoggingWebSocket {
	...
	constructor(url: string) {
		this.socket = new WebSocket(`${url}?token=${getJwtToken()}`); // instanse hosil qilyapmiz
		socketVar(this.socket); // socket client instanse saqlayapmiz
...
function createIsomorphicLink() {
	if (typeof window !== 'undefined') {
		...
		const wsLink = new WebSocketLink({
			uri: process.env.REACT_APP_API_WS ?? 'ws://127.0.0.1:8000',
			options: {
				reconnect: false,
				timeout: 30000,
				connectionParams: () => {
