...
const PropertyDetail: NextPage = ({ initialComment, ...props }: any) => {
	...
	useEffect(() => {
		if (router.query.id) {
			...
			setCommentInquiry({
				...commentInquiry,
				search: {
					commentRefId: router.query.id as string, // aynan qaysi propertyga comment
				},
			});
	...
	const likePropertyHandler = async (user: T, id: string) => {
		try {
			...
			}); // page refresh bolganda ham like turoradi
	...
	const createCommentHandler = async () => {
		try {
			if (!user._id) throw new Error(Message.NOT_AUTHENTICATED); // Auth bolganmi ?
			await createComment({ variables: { input: insertCommentData } }); // insertNewComment

			setInsertCommentData({ ...insertCommentData, commentContent: '' }); // keyinchalik yana comment yoziw un bo`w stringga aylantiryapmiz

			await getCommentsRefetch({ input: commentInquiry }); // hamma commentlarni refetch qilib ovolyapmiz (retrieve)
		} catch (err: any) {
			// sweetErrorHandling(err).then(); // or below cz of *async* funct
